...
# from ..fmsketch import intersection_ca
from PrivMRF.fmsketch import intersection_ca
from PrivMRF.factor import Factor
...
def one_round_intersection_alpha(attr_pair, idx, fmsketches):
    # compute the sketch of the union of complementary
    sketch = []
    for i in range(2):
        sketch.append(fmsketches[idx][attr_pair[i]]['com_sketch'])
...
def fm_intersection_ca(config, attr_pair,domain, fmsketches, noisy_data_num,eps):
    ...
    num_intersections = np.product([domain.dict[attr_pair[0]]['domain'],domain.dict[attr_pair[1]]['domain']])
    all_sketches = np.zeros(shape=(m, num_intersections))

    for idx in range(m):
        all_sketches[idx] =  one_round_intersection_alpha(attr_pair, idx, fmsketches)
    debias = 0.7213 / (1 + 1.079 / m)
    
    # epsilon, delta = priv_config['eps'], priv_config['delta']
    domain_size = domain.dict[attr_pair[0]]['domain']
    c = 2*(domain_size-1)
    # len(splits) * (len(splits[0]) - 1)
    k_p, _ = set_k_p_min(eps, 1/noisy_data_num, m, gamma)
    ...
    estimate = noisy_data_num - raw_comlementary_union
    # print(f"DP estimate: {estimate}")
    return estimate.reshape(domain.dict[attr_pair[0]]['domain'], \
                                        domain.dict[attr_pair[1]]['domain'])

def fm_histogram_ca(config, attr, domain, fmsketches, noisy_data_num,eps,priv_config=True):
    m = config['m']
    gamma = config['gamma']
    # domain_size = self.domain.dict[attr]['domain']
    one_way_sketches = []
    for idx in range(m):
        one_way_sketches.append(fmsketches[idx][attr]['sketch'])
    ...
    histogram = m / np.sum(np.power(1 + gamma, -np.array(one_way_sketches)), axis=0) * debias - k_p
        # one_ways.append(raw_estimate)
    return histogram


def dp_TVD(config, TVD_map, domain, index_list, fmsketches, noisy_data_num,eps,priv_config=True,gpu=True):
    ...
    if index_list not in TVD_map:
        ...
        histogram = fm_intersection_ca(config, index_list,domain, fmsketches, noisy_data_num,eps)
        fact1 = Factor(domain, histogram, np)
        temp_domain = domain.project([index_list[0]])
        histogram= fm_histogram_ca(config, index_list[0], domain, fmsketches, noisy_data_num,eps,priv_config=True)
        fact2 = Factor(temp_domain, histogram, np)
        temp_domain = domain.project([index_list[1]])
        histogram= fm_histogram_ca(config, index_list[0], domain, fmsketches, noisy_data_num,eps,priv_config=True)
        fact3 = Factor(temp_domain, histogram, np)
        fact4 = fact2.expand(domain) * fact3.expand(domain) / noisy_data_num
        TVD = np.sum(np.abs(fact4.values - fact1.values)) / 2 / noisy_data_num
        if gpu:
            TVD = TVD.item()
        # noisy_TVD = TVD + np.random.normal(scale=noise)
        TVD_map[index_list] = [TVD, TVD]
...
def dp_mutual_info(MI_map, entropy_map, data, domain, index_list, noise):
    ...
    if index_list not in MI_map:
        ...
        noisy_MI = MI + np.random
        MI_map[index_list] = [MI, noisy_MI]
        # print(index_list, MI)

    return MI_map[index_list]

def dp_entropy(config, entropy_map, fmsketches, domain, index_list, noisy_data_num,eps):
    if not isinstance(index_list, tuple):
        index_list = tuple(sorted(index_list))
    if index_list not in entropy_map:
        ...
        # if len(index_list) <= 12 and size < 5e6:
            # histogram = np.histogramdd(data[:, index_list], bins=bins)
        histogram = fm_intersection_ca(config, index_list,domain, fmsketches, noisy_data_num,eps)
        ...
        if entropy < 0:
            entropy = 0
        entropy_map[index_list] = [entropy, entropy]
...
def one_round_intersection_alpha(fmsketches, attr_pair, idx):

    sketch = []
    for i in range(len(attr_pair)):
        sketch.append(fmsketches[idx][attr_pair[i]]['private_statistics'])
    cartesian = list(itertools.product(*sketch))
    return [np.max(c) for c in cartesian]


def fm_intersection_ca(data, fmsketches, config, attr_pair, domain, eps, noisy_data_num):
    # assert multithreads >= 1
    # if (not priv_config is None) :
    #     assert 'eps' in priv_config and 'delta' in priv_config
    #     if not 'm' in priv_config:
    ...
    gamma = config['gamma']
    # seeds = np.random.randint(0, high=100000, size=m)
    histogram = clean_intersection_ca(data, attr_pair, domain)

    num_intersections = np.product([domain.dict[attr_pair[0]]['domain'],domain.dict[attr_pair[1]]['domain']])
    all_sketches = np.zeros(shape=(m, num_intersections))

    for idx in range(m):
        all_sketches[idx] = one_round_intersection_alpha(fmsketches, attr_pair, idx)
    debias = 0.7213 / (1 + 1.079 / m)
    ...
    estimate = estimate * noisy_data_num/np.sum(estimate)
    DP_FM_histogram = estimate.reshape(domain.dict[attr_pair[0]]['domain'], \
                                        domain.dict[attr_pair[1]]['domain'])
    # print(f"DP estimate: {estimate}")
    return DP_FM_histogram


def clean_intersection_ca(data, attr_pair, domain):
    domain = domain.project(attr_pair)
    bins = domain.edge()
    histogram, _= np.histogramdd(data[:, attr_pair], bins=bins)
    return histogram


def clean_histogram_ca(data, attr, domain):
...
def fm_histogram_ca(data, fmsketches, config, attr, domain, eps, noisy_data_num):
    m = config['m']
    gamma = config['gamma']
    c = domain.dict[attr]['domain']-1
    clean_estimate = clean_histogram_ca(data, attr, domain)
    # domain_size = self.domain.dict[attr]['domain']
    one_way_sketches = []
    for idx in range(m):
        one_way_sketches.append(fmsketches[idx][attr]['private_statistics'])
    # estimate one party's ca
    debias = 0.7213 / (1 + 1.079 / m)
    # one_ways = []
    # if self.priv_config:
    ...
    estimate = noisy_data_num - complementary_estimate
    estimate[estimate< 0] = 0
    estimate  = estimate * noisy_data_num/np.sum(estimate)
        # one_ways.append(raw_estimate)
    return estimate
...
def rr_intersection_ca(random_responses,attr_pair, domain, eps):
    domain_size_0 = domain.dict[attr_pair[0]]['domain']
    domain_size_1 = domain.dict[attr_pair[1]]['domain']
    n, d = random_responses[attr_pair[0]].shape
    rr_intersection_histogram = np.zeros((domain_size_0,domain_size_1))
    for i in range(domain_size_0):
        for j in range(domain_size_1):
            vector_0 = (random_responses[attr_pair[0]][:,i]-1/2+eps)/(2*eps)
            vector_1 = (random_responses[attr_pair[1]][:,j]-1/2+eps)/(2*eps)
...
def rr_intersection_dic(random_responses,attr_list,domain, eps):
    rr_intersection_histogram_dict={}
    attr_list_temp = attr_list.copy()
    for attr1 in attr_list_temp:
        for attr2 in attr_list_temp:
            if attr2 > attr1:
               rr_intersection_histogram_dict[(attr1,attr2)] = rr_intersection_ca(random_responses,[attr1,attr2], domain, eps)
    return rr_intersection_histogram_dict


def rr_histogram_ca(data, attr_list, random_responses,attr, domain, rr_intersection_histogram_dict):
    temp_dict = rr_intersection_histogram_dict.copy()
    domain_size = domain.dict[attr]['domain']
    n, d = random_responses[attr].shape
    num_other_attr = len(attr_list)-1
...
########################################### compute the noisy R-scores

def dp_TVD(data, private_statistics, config, index_list, domain, eps, noisy_data_num, attr_list, rr_intersection_histogram_dict):
    domain = domain
    TVD_map = {}
    if not isinstance(index_list, tuple):
        index_list = tuple(sorted(index_list))
    if index_list not in TVD_map:
        ...
        if config['private_method'] == 'fmsketch':
            histogram = fm_intersection_ca(data, private_statistics, config, index_list, domain, eps, noisy_data_num)
            fact1 = Factor(domain, histogram, np)
            temp_domain = domain.project([index_list[0]])
            histogram= fm_histogram_ca(data, private_statistics, config, index_list[0], domain, eps, noisy_data_num)
            fact2 = Factor(temp_domain, histogram, np)
            temp_domain = domain.project(data, private_statistics, config, index_list[1], domain, eps, noisy_data_num)
            histogram= fm_histogram_ca(index_list[1])
            fact3 = Factor(temp_domain, histogram, np)
        else:
            histogram = rr_intersection_histogram_dict[(index_list[0],index_list[1])]
            fact1 = Factor(domain, histogram, np)
            temp_domain = domain.project([index_list[0]])
            histogram= rr_histogram_ca(data, attr_list, private_statistics,index_list[0], domain, rr_intersection_histogram_dict)
            fact2 = Factor(temp_domain, histogram, np)
            temp_domain = domain.project([index_list[1]])
            histogram= rr_histogram_ca(index_list[1])
            fact3 = Factor(temp_domain, histogram, np)
        fact4 = fact2.expand(domain) * fact3.expand(domain) / noisy_data_num
        TVD = np.sum(np.abs(fact4.values - fact1.values)) / 2 / noisy_data_num
        if gpu:

...
def svm_classifier(exp_name, data_name, dp_data_list, classifier_num, target_list, \
    print_lock, result_lock, epsilon, cross_valid_round, target_parallel):
    data = []
    for k in range(5):
    ...
    classifier_num = min(attr_num, len(target_list))
    ...
    # data_name = 'adult1'
    result_lock.acquire()
    if os.path.exists('./result/svm/'+data_name+str(epsilon)+exp_name+'_SVM.json'):
        with open('./result/svm/'+data_name+str(epsilon)+exp_name+'_SVM.json', 'r') as in_file:
    ...
    with open('./result/svm/'+data_name+str(epsilon)+exp_name+'_SVM.json', 'w') as out_file:
